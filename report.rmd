...
#read model
readRDS(file="./models/lm_train.rda") 
...
Using kNN requires to choose the right k, meaning to define the size of the groups of observations that will be combined as one prediction. First tries resulted in aborted simulations due to too many neighbors (>1000).  
Running SVM on the full training data set resulted in a crash of R. In order to have a basic model available we reduced the p-value for both models to 0.05, so only 5% of the training data set. 

The results are much worse compared to the other approaches.
...
# load model to use it in the markup file
loadRDS(cart_train, file="./models/cart_train.rda")
...
To find the best random forest we also use a tuning run, using the rborist package and finding the lowest RMSE for cp and amount of randomly selected predictors. Below you can see the performance comparison for different values as well as the results for the best performing model. The final RMSE is added to the comparison table. 
```{r random forest}
#### MODEL 3.2 - RANDOM FOREST #### 

#load model data of first run with CV and different parameters
readRDS(file="./models/train_rf.rda")
...
#load model with best tuning parameters from above
readRDS(file="./models/rf_train.rda")
